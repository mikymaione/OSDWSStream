...
namespace webpp
{
    ...
    public:
        ...
        public:
            ...
            explicit Connection(const std::shared_ptr<socket_type> &socket) : super(0), socket(socket),
                                                                              strand(socket->get_io_service()),
                                                                              closed(false)
            {}

        private:
            explicit Connection(socket_type *socket) : super(0), socket(socket), strand(socket->get_io_service()),
                                                       closed(false)
            {}
            ...
            {
            public:
                SendData(const std::shared_ptr<SendStream> &header_stream,
                ...
                strand.post([this, connection]()
                            {
                                asio::async_write(*socket, send_queue.begin()->header_stream->streambuf,
                                                  strand.wrap([this, connection](const std::error_code &ec,
    ...
    private:
        ...
        public:
            regex_orderable(const char *regex_cstr) : std::regex(path2regex::path_to_regex(regex_cstr, keys)),
                                                      str(regex_cstr)
            {}

            regex_orderable(const std::string &regex_cstr) : std::regex(path2regex::path_to_regex(regex_cstr, keys)),
                                                             str(regex_cstr)
            {}
    ...
    public:
        ...
        virtual void start()
        {
            ...
            auto endpoint = config.address.empty() ?
                            asio::ip::tcp::endpoint(asio::ip::tcp::v4(), config.port) :
                            asio::ip::tcp::endpoint(asio::ip::address::from_string(config.address), config.port);
    ...
    protected:
        ...
        std::shared_ptr<asio::system_timer>
        get_timeout_timer(const std::shared_ptr<Connection> &connection, size_t seconds)
        {
            if (seconds == 0)
                return nullptr;
                    ...
                    while ((param_end = line.find(':')) != std::string::npos)
                    {
                        ...
                        if ((value_start) < line.size())
                        {
                            if (line[value_start] == ' ')
                                value_start++;
                            if (value_start < line.size())
                                connection->header.emplace(line.substr(0, param_end),
        ...
        void write_handshake(const std::shared_ptr<Connection> &connection,
                             const std::shared_ptr<asio::streambuf> &read_buffer)
        {
                                 ...
                                 if (!ec)
                                 {
                                     ...
                                     if (first_bytes[1] < 128)
                                     {
                                         ...
                                         send_close(connection, 1002, reason,
                                                    [connection](const std::error_code & /*ec*/)
                                                    {});
                                         ...
                                         asio::async_read(*connection->socket, *read_buffer, asio::transfer_exactly(2),
                                                                   ...
                                                                   size_t /*bytes_transferred*/)
                                                          {
                                                              if (!ec)
                                                              {
                                                                  ...
                                                                  stream.read(
                                                                          reinterpret_cast<char *>(&length_bytes[0]),
                                                                          2);

                                                                  size_t length = 0;
                                                                  int num_bytes = 2;
                                                                  for (int c = 0; c < num_bytes; c++)
                                                                      length += length_bytes[c]
                                                                              << (8 * (num_bytes - 1 - c));

                                                                  read_message_content(connection, read_buffer, length,
                                     ...
                                     else if (length == 127)
                                     {
                                         ...
                                         read_message_content(connection, read_buffer, length, endpoint,
                                                              fin_rsv_opcode);
                                 }
                                 else
                                     connection_error(connection, endpoint, ec);
        ...
        void timer_idle_init(const std::shared_ptr<Connection> &connection)
        {
            if (config.timeout_idle > 0)
            {
                ...
                connection->timer_idle->expires_from_now(
                        std::chrono::seconds(static_cast<unsigned long>(config.timeout_idle)));
        ...
        {
            if (config.timeout_idle > 0 && connection->timer_idle->expires_from_now(
                    std::chrono::seconds(static_cast<unsigned long>(config.timeout_idle))) > 0)
                timer_idle_expired_function(connection);
        }
        ...
        void timer_idle_expired_function(const std::shared_ptr<Connection> &connection) const
        {
            connection->timer_idle->async_wait([this, connection](const std::error_code &ec)
                                               {
                                                   if (!ec)
                                                       send_close(connection, 1000,
                                                                  "idle timeout"); //1000=normal closure
                                               });
        }
    };
